...
//默认包大小为200
uint32_t packet_size = sizeof(struct bob_msg);
...
int main(int argc, char** argv)
{
	...
	*/
	...
	//2. allocat packet and set header
	...
	struct bob_msg *msg = malloc(sizeof(struct bob_msg));
	if(msg == NULL){
		printf("malloc bob_msg error\n");
		return -1;
	}
	msg->start_byte = START_BYTE;
	msg->end_byte = END_BYTE;
	msg->command = CSI_COMMAND;
	memcpy(packet->payload,msg,sizeof(struct bob_msg));
	send_beacon(seq);
